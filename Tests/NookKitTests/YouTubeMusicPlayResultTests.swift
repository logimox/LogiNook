...
final class YouTubeMusicPlayResultTests: XCTestCase {
    func testQueueRequestAddsSelectedVideoAtEnd() throws {
        let client = YouTubeMusicClient(baseURL: URL(string: "http://127.0.0.1:26538")!)
        let request = try client.queueRequest(videoID: "5NV6Rdv1a3I")
        ...
        XCTAssertEqual(body?["insertPosition"], "INSERT_AT_END")
    }

    func testPlayNowRequestAddsSelectedVideoAfterCurrentTrack() throws {
        let client = YouTubeMusicClient(baseURL: URL(string: "http://127.0.0.1:26538")!)
        let request = try client.playNowRequest(videoID: "5NV6Rdv1a3I")
